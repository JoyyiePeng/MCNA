...
class MoEMixin:
    ...
    def get_aux_loss(self):
        ...
        for module in self.modules():
            if hasattr(module, 'get_aux_loss') and module is not self:
    ...
    def get_expert_usage(self):
        ...
        for name, module in self.named_modules():
            if hasattr(module, 'get_expert_usage') and module is not self:
                # 需要特殊处理，因为 get_expert_usage 需要输入
                if hasattr(module, '_last_expert_stats'):
                    stats[name] = module._last_expert_stats
        return stats
    
    def reset_aux_loss(self):
        """重置所有 MoE 层的辅助损失（每个 batch 后调用）"""
        for module in self.modules():
...
#                  activation='ReLU', use_ncn=True, ncn_layer_idx=1, **kwargs):
#         super().__init__()
...
#         # Build layers with optional NCN enhancement
#         for i in range(num_layers):
#             input_dim = in_feats if i == 0 else h_feats
#             if use_ncn and i == ncn_layer_idx:
#                 # Add NCN-enhanced layer
#                 conv_layer = dglnn.GraphConv(input_dim, h_feats, activation=self.act)
#                 ncn_layer = NCNLayer(input_dim, h_feats, dropout=dropout_rate, activation=activation)
#                 self.layers.append(CombinedNCNLayer(conv_layer, ncn_layer))
#             else:
...
class GCN(MoEMixin, nn.Module):  # 继承 MoEMixin
    def __init__(self, in_feats, h_feats=32, num_classes=2, num_layers=2, mlp_layers=1, dropout_rate=0,
                 activation='ReLU', use_ncn=True, ncn_layer_idx=1,**kwargs):
        super().__init__()
        self.h_feats = h_feats
...
#     def __init__(self, num_layers=2, agg='mean', init_eps=-1, use_ncn=True, ncn_layer_idx=1, **kwargs):
#         super().__init__()
...
#         # Initialize NCN layer if needed (will be created dynamically in forward pass)
#         self.ncn_layer = None
...
#             # Apply NCN enhancement at the specified layer
#             if self.use_ncn and i == self.ncn_layer_idx:
#                 # Create NCN layer dynamically based on current feature dimension
#                 if self.ncn_layer is None:
#                     feat_dim = h.shape[1]
#                     self.ncn_layer = NCNLayer(
#                         in_feats=feat_dim,
#                         out_feats=feat_dim
#                     ).to(h.device)
                
#                 # Apply NCN enhancement to current layer output
#                 h_ncn = self.ncn_layer(graph, h)
#                 # Concatenate both GIN output and NCN enhanced output
#                 h_final = torch.cat([h_final, h, h_ncn], -1)
#             else:
...
class GIN_noparam(nn.Module):
    def __init__(self, num_layers=2, agg='mean', init_eps=-1, use_ncn=True, ncn_layer_idx=0, **kwargs):
        super().__init__()
        self.gnn = dglnn.GINConv(None, activation=None, init_eps=init_eps,
                                 aggregator_type=agg)
        self.num_layers = num_layers
            ...
            if self.use_ncn and i == self.ncn_layer_idx:
                ...
                if self.moe_layer is None:
                    ...
                    self.moe_layer = SparseMultiHopMoE(
                        gcn_layer=self.gnn,  # 直接复用 self.gnn
                        in_feats=feat_dim,
                        out_feats=feat_dim,
                        num_hops=3,
                        top_k=1,
                ...
                # MoE 内部会调用 self.gnn，所以这层不需要再单独调用
                h = self.moe_layer(graph, h)
            else:
                h = self.gnn(graph, h)
    ...
    def get_aux_loss(self):
        """收集所有 MoE 层的辅助损失"""
        total_aux_loss = 0.0
        total_aux_loss += self.moe_layer.get_aux_loss()
...
#         for i in range(num_layers-1):
#             if use_ncn and i == (ncn_layer_idx-1):
#                 ncn_layer = NCNLayer(h_feats, h_feats, dropout=dropout_rate, activation=activation)
#                 self.layers.append(CombinedNCNLayer(dglnn.SAGEConv(h_feats, h_feats, agg, activation=self.act), ncn_layer))
#             else:
...
class GraphSAGE(nn.Module):
    def __init__(self, in_feats, h_feats=32, num_classes=2, num_layers=2, agg='mean', dropout_rate=0,
        ...
        for i in range(num_layers-1):
            if use_ncn and i == (ncn_layer_idx-1):
                combined_layer = SparseMultiHopMoE(
                    gcn_layer=dglnn.SAGEConv(h_feats, h_feats, agg, activation=self.act),
                    in_feats=h_feats,
                    out_feats=h_feats,
                    num_hops=3,      # 1-hop, 2-hop, 3-hop
                    top_k=kwargs.get('moe_top_k', 1),
